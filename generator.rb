...
def render_card_page(pdf, card_geometry, icon, statements, is_black, game_info)
	...
	statements.each_with_index do |line, idx|
		box(pdf, card_geometry, idx) do
			...
			is_warn = false
			warn_text = ""
			if card_text.include? "((" and card_text.include? "))"
				is_warn = true
			...
			#pick 2
			if is_pick2 and !is_warn
				pdf.text_box "PICK", size:11, align: :right, width:30, at: [pdf.bounds.right-50,pdf.bounds.bottom+20+1]
				...
				pdf.stroke_color '000000'
				pdf.fill_color '000000'
				pdf.text_box "2", color:"000000", size:14, width:8, align: :center, at:[pdf.bounds.right-14+4,pdf.bounds.bottom+21+1.5]
				pdf.stroke_color "ffffff"
				pdf.fill_color "ffffff"
			end
			...
			if is_pick3 and !is_warn
				...
				pdf.stroke_color '000000'
				pdf.fill_color '000000'
				pdf.text_box "3", color:"000000", size:14, width:8, align: :center, at:[pdf.bounds.right-14+4,pdf.bounds.bottom+21+1.5]
				pdf.stroke_color "ffffff"
				pdf.fill_color "ffffff"


				pdf.text_box "DRAW", size:11, align: :right, width:35, at: [pdf.bounds.right-55,pdf.bounds.bottom+40+1]
				pdf.fill_and_stroke(:fill_color=>"ffffff", :stroke_color=>"ffffff") do
					pdf.circle([pdf.bounds.right-10+4,pdf.bounds.bottom+35.5+2],7.5)
				end
				pdf.stroke_color '000000'
				pdf.fill_color '000000'
				pdf.text_box "2", color:"000000", size:14, width:8, align: :center, at:[pdf.bounds.right-14+4,pdf.bounds.bottom+41+1.5]
				pdf.stroke_color "ffffff"
				pdf.fill_color "ffffff"
			end
			...
			if is_warn
				if is_black
					...
					pdf.stroke_color '000000'
					pdf.fill_color '000000'
					pdf.text_box warn_text, color:"000000", size:14, width:12, align: :center, at:[pdf.bounds.right-14+4-2,pdf.bounds.bottom+21+1.5]
					...
					pdf.stroke_color 'ffffff'
					pdf.fill_color 'ffffff'
					pdf.text_box warn_text, color:"ffffff", size:14, width:12, align: :center, at:[pdf.bounds.right-14+4-2,pdf.bounds.bottom+21+1.5]
					pdf.stroke_color stroke_previous
					pdf.fill_color fill_previous
				end
			end
			...
			if is_pick2 or is_pick3
				text = game_info[:game_name_abbr]
			end
...
def load_info(file)
	...
	return { :game_name => name, :game_name_abbr => name_abbr, :game_version => version }
end
	...
	if (not input_files_are_absolute) and recurse
		...
		files_in_dir.each do |subdir|
			if File.directory? subdir
				render_cards(subdir, original_info_file, original_white_file, original_black_file, original_icon_file, "irrelevant", false, true, true, card_geometry )
			end
		end
	end
...
"
	...
	puts "-----------------------------------------------------------"
	puts ""
	puts "https://dev.bloomgogo.com/projects/cah-generator"
	puts ""
...
def main
	...
	if args.has_key? "help" or args.length == 0 or ( (not args.has_key? "white") and (not args.has_key? "black") and (not args.has_key? "dir") )
		...
		render_cards args["dir"], "info.txt", "white.txt", "black.txt", "icon.png", "cards.pdf", false, true, true, card_geometry, "", "", false
	else
